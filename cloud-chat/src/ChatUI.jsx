...
function ChatUI({ user }) {
  ...
  useEffect(() => {
    socketRef.current = io('http://10.217.26.188:5001');
    return () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`http://10.217.26.188:5001/api/chats?username=${user}`)
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    fetch(`http://10.217.26.188:5001/api/messages/${chatId}?username=${user}`)
      .then(res => res.json())
      .then(data => {
        setMessages(data);
        setMsgLoading(false);
  ...
  const handleSend = async (e) => {
    ...
    if (input.trim() && selectedContact) {
      const chatId = getChatId(user, selectedContact);
      try {
        const res = await fetch('http://10.217.26.188:5001/api/messages', {
          method: 'POST',
  ...
  // (Removed duplicate handleDeleteChat)
    ...
    const handleDeleteChat = async () => {
      ...
      await fetch('http://10.217.26.188:5001/api/messages/deleteForUser', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (newChatName && !contacts.some(c => c.name.toLowerCase() === newChatName.toLowerCase())) {
                    try {
                      const res = await fetch('http://10.217.26.188:5001/api/chats', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
